...
/* File: core_main.c
        This file contains the framework to acquire a block of memory, seed initial parameters, tun t he benchmark and
   report the results.
*/
#include "coremark.h"
...
/* Function: main
        ...
        3 - third seed  : Any value, should be at least an order of magnitude less then the input size, but bigger
   then 32. 4 - Iterations  : Special, if set to 0, iterations will be automatically determined such that the benchmark
   will run between 10 to 100 secs
...
MAIN_RETURN_TYPE CoreMark(void)
{
    ...
    char* argv[1];
#else
MAIN_RETURN_TYPE CoreMark(int argc, char* argv[])
{
#endif
...
#error "Please define a way to initialize a memory block."
    ...
    if (results[0].iterations == 0)
    {
        ...
        if (divisor == 0) /* some machines cast float to int as 0 since this conversion is not defined by ANSI, but we
                             know at least one second passed */
            divisor = 1;
